<?php
	class EltakoFWS61 extends IPSModule
	{
		...
		#=====================================================================================
		{
	        ...
	        switch($data->Device) {
	            case "165":
					if($data->DataByte0 == "24" OR $data->DataByte0 == "26") { // Telegrammteil 1
						$this->SetValue('Dawn', (int)$data->DataByte3 * (1000/255));
						$temp = ((int)$data->DataByte2 * (120/255))
						if ($temp > 40) {
							$temp = round(($temp - 40)*2)/2;
							$this->SetValue('Temperature', $temp); //Wenn Wert ueber 40
						}
						else {
							$temp = round((-40 + $temp)*2)/2;
							$this->SetValue('Temperature', $temp); //Wenn Wert unter 40
						 }
					}
